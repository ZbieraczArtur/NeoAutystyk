...
(function () {
  ...
  // kolejność pytań bez warunków, a następnie wstawiamy każde aktywne pytanie
  // bezpośrednio za ostatnim pytaniem, od którego zależy. Dzięki temu warunek
  // z wielu pytań jest widoczny dopiero po ostatnim z nich.
  function orderedActiveQuestions() {
    const active = config.questions.filter(isQuestionVisible);
    const byId = new Map(active.map(question => [idOf(question.id), question]));
    const ordered = active.filter(question => conditionIds(question).length === 0);
    const pending = active.filter(question => conditionIds(question).length > 0);

    while (pending.length) {
      let inserted = false;
      for (let index = 0; index < pending.length; index++) {
        const question = pending[index];
        const dependencies = conditionIds(question).filter(id => byId.has(id));
        const positions = dependencies.map(id => ordered.findIndex(item => idOf(item.id) === id));
        if (positions.some(position => position < 0)) continue;
        ordered.splice(Math.max(...positions) + 1, 0, question);
        pending.splice(index, 1);
        inserted = true;
        break;
      }
      // Zabezpieczenie przed błędną lub cykliczną konfiguracją warunków:
      // aplikacja nadal pokazuje pytanie zamiast wpadać w nieskończoną pętlę.
      if (!inserted) {
  ...
  function updateReviewControls(activeQuestions = orderedActiveQuestions()) {
    const unanswered = activeQuestions.filter(question => answerState(question) === 'unanswered').length;
    const skipped = activeQuestions.filter(question => answerState(question) === 'skipped').length;
    const questionWord = count => count === 1 ? 'pytanie' : (count % 10 >= 2 && count % 10 <= 4 && (count % 100 < 10 || count % 100 >= 20) ? 'pytania' : 'pytań');
    const status = document.getElementById('question-review-status');
    const unansweredButton = document.getElementById('review-unanswered-btn');
    const skippedButton = document.getElementById('review-skipped-btn');
    const allButton = document.getElementById('review-all-btn');
    if (unansweredButton) { unansweredButton.textContent = `Pokaż pytania bez odpowiedzi (${unanswered})`; unansweredButton.disabled = unanswered === 0; }
    if (skippedButton) { skippedButton.textContent = `Pokaż pytania oznaczone „Pomiń” (${skipped})`; skippedButton.disabled = skipped === 0; }
    if (allButton) allButton.hidden = !reviewFilter;
    if (status) {
      status.textContent = reviewFilter === 'unanswered'
        ? `Wyświetlasz ${unanswered} aktywne ${questionWord(unanswered)} bez odpowiedzi.`
        : reviewFilter === 'skipped'
          ? `Wyświetlasz ${skipped} aktywne ${questionWord(skipped)} oznaczone jako „Pomiń”.`
          : `Aktywne pytania: ${activeQuestions.length}. Bez odpowiedzi: ${unanswered}. Oznaczone „Pomiń”: ${skipped}.`;
    }
  }
  ...
  document.getElementById('review-unanswered-btn')?.addEventListener('click', () => setReviewFilter('unanswered'));
  document.getElementById('review-skipped-btn')?.addEventListener('click', () => setReviewFilter('skipped'));
  document.getElementById('review-all-btn')?.addEventListener('click', () => setReviewFilter(null));
  window.resetQuestionReview = () => { reviewFilter = null; };
  ...
  function renderModernQuestions() {
    ...
    const activeQuestions = orderedActiveQuestions();
    updateReviewControls(activeQuestions);
  ...
  function renderComparison() {
    ...
    let actions = panel.querySelector('.comparison-actions');
    if (!actions) { actions = document.createElement('div'); actions.className = 'comparison-actions'; body.after(actions); }
    panel.querySelectorAll('[data-comparison-limit], [data-comparison-expand]').forEach(button => actions.appendChild(button));
